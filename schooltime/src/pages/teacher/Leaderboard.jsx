          ...
          : (
            <div className="card overflow-hidden">
              ...
              {board.slice(0, 3).length > 0 && (
                <div className="flex items-end justify-center gap-4 p-8 bg-gradient-to-br from-brand-50 to-white border-b border-gray-100">
                  {[board[1], board[0], board[2]].filter(Boolean).map((entry, i) => {
                    ...
                    return (
                      <div key={entry.student_id} className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-sm font-semibold text-gray-800 text-center leading-tight">{entry.full_name.split(' ')[0]}</p>
                        <p className="text-lg font-bold text-brand-600">{entry.score_pct}%</p>
                        <div className={`bg-brand-600 rounded-t-lg w-16`} style={{ height: `${h * 2}px` }} />
                      </div>
                    )
                  })}
                </div>
